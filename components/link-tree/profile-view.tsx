...
export function ProfileView({ profile, links }: ProfileViewProps) {
  ...
  return (
    ...
    >
      <CardContent className="p-6 px-6 border-0 leading-7 mx-5">
        <div className={cn("flex flex-col items-center space-y-4", themeSettings.font)}>
          ...
          <div className="text-center">
            <div className="flex items-center justify-center gap-1.5 leading-7 flex-row">
              <h2 className="text-xl font-extrabold leading-7 tracking-normal" style={{ color: "var(--font-color-display-name)" }}>
                {profile.name}
              </h2>
              {profile.verified && <VerifiedBadge />}
            </div>
            <p className="mt-1 text-sm max-w-md font-medium" style={{ color: "var(--font-color-bio)" }}>
              {profile.bio}
            </p>
          </div>
